...
- Exact finding IDs encode diff coordinates rather than content, and an unchanged hunk still does not prove that nondeterministic replicas produced the same diagnosis set. Tier-one carry therefore compares SHA-256 digests of both the canonical hunk text and the complete order-insensitive set of collapsed finding bodies. A changed or unavailable digest demotes to tier two, while an empty body set is corrupt merge state and fails as an invariant. Verdicts written with the older first-body digest remain readable but conservatively demote when that digest differs. Duplicate `(file, rule_id)` pairs are counted before exact matching across all inherited findings and all current actionable findings, so exact IDs can never bypass ambiguity.
...
- Resume without dispositions is a request to pause only while a run is unfinished. If the run already has a completed verdict, replacing it with a findings-empty pause stub would destroy validated diagnostic evidence, so gate returns `verdict_already_completed` and directs the operator to create a new target run with `--inherit`. Existing pause stubs remain replaceable.
- Authorization subprocess failures are evidence-bearing gate outcomes: the BLOCK artifact records affected blockers, the lookup step, a resolved actor when actor lookup completed, and a bounded diagnostic. Because the same detail fans out to console, JSON, Markdown, and CI logs, redaction occurs once when subprocess stderr enters `GitHubAuthorizationError`; token forms, authorization values, bearer values, long encoded runs, and control characters are removed before any consumer sees the text, and oversized detail is explicitly truncated at 500 characters.
...
- Legacy verdicts may lack hunk or body digests, may contain the older first-body digest, and may lack per-finding inheritance diagnostics. They remain readable, but exact-ID findings are conservatively handled as tier two whenever the required current digest bindings do not match.
