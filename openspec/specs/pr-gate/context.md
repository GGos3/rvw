...
- Exact finding IDs encode diff coordinates rather than content, and an unchanged hunk still does not prove that nondeterministic replicas produced the same diagnosis set. Tier-one carry therefore compares SHA-256 digests of both the canonical hunk text and the complete order-insensitive set of collapsed finding bodies. Body identity uses an outer SHA-256 over the concatenated raw inner SHA-256 blocks of bodies in sorted order, avoiding delimiter ambiguity. Missing source and current digests, changed hunk content, and changed diagnosis bodies demote as `source_digest_missing`, `current_digest_missing`, `content_changed`, and `diagnosis_changed`. Older first-body and delimiter-joined digests remain readable but conservatively demote as changed diagnoses. An empty body set is corrupt merge state and persists a correlated invariant BLOCK verdict. Duplicate `(file, rule_id)` pairs are counted before exact matching across all inherited findings and all current actionable findings, so exact IDs can never bypass ambiguity.
...
- A verdict with actionable counts but no finding records is the pause stub written before dispositions are validated, not a carry source. Inheritance rejects it and directs the operator to finish that run first; a clean verdict with no actionable counts remains valid. At this source boundary, counts must have exactly the three verdict keys with integer values, and accepted reasons must be nonblank even though the historical artifact model stays permissive.
- Run IDs are restricted to a conservative ASCII direct-child alphabet before any filesystem lookup. Inheritance artifact reads validate parent containment, open the final component with no-follow semantics, verify the opened descriptor is regular, and parse JSON from that same descriptor. Resume rejects self-inheritance before loading to preserve acyclic provenance and the source evidence.
- A resume is valid only while a run is unfinished. If the run already has a completed verdict, any resume—including one with a fully covering inheritance source or a supplied disposition document—returns `verdict_already_completed` before inheritance or disposition work and leaves the evidence byte-identical. Existing pause stubs remain replaceable.
- Authorization subprocess failures are evidence-bearing gate outcomes: the BLOCK artifact records affected blockers, the lookup step, a resolved actor when actor lookup completed, and a bounded diagnostic. Because the same detail fans out to console, JSON, Markdown, and CI logs, redaction occurs once when subprocess stderr enters `GitHubAuthorizationError`; token forms, authorization values, bearer values, long encoded runs, C0/C1 controls, and Unicode format controls are removed before any consumer sees the text, and oversized detail is explicitly truncated at 500 characters.
