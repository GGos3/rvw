...
Gate MUST require a nonempty activated lane plan, exactly one coverage row per planned lane, no unplanned coverage row, a positive dispatched count equal to the configured replica count for every lane, and `valid == dispatched` for every lane.

#### Scenario: Vacuous run has no dispatches

- **WHEN** discovery contains no coverage rows or a lane reports zero dispatched replicas
...
#### Scenario: A lane has an invalid replica

- **WHEN** a planned lane reports fewer valid replicas than dispatched replicas
- **THEN** gate fails with that lane's validity in the verdict artifact
