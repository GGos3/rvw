...
The merge stage MUST collapse findings by the tuple `(file, hunk_id, rule_id)` and MUST expose the lowercase SHA-1 digest of `file:hunk_id:rule_id` as the group key.
