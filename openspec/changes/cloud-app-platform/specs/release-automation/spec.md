...
other path from running. A cloud deployment job MAY run in parallel with publication
jobs only when repository variable `RVW_CLOUD_DEPLOY` is exactly `true`; it MUST deploy
the production Worker before applying planned Terraform resources.
...
- **THEN** `deploy-cloud` deploys Wrangler `prod` successfully before running `terraform apply -auto-approve -var environment=prod`
