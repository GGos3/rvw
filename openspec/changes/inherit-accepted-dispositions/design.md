...
`--inherit <run-id>` loads the prior run's `gate-verdict.json` (not its raw `gate-dispositions.yaml`): the verdict is the only artifact whose dispositions passed exact-ID, duplicate, unknown, and owner-authorization validation. After legacy kind inference, only a `completed` verdict is a source; typed pause and failure artifacts are incomplete even if they contain accepted finding records. The source run's `target.json` must match the current repository and PR number, comparing the GitHub slug case-insensitively while keeping the PR number exact; head/base anchors MAY differ (that is the point). Artifact-derived observed identity strings are redacted before mismatch reporting. Missing run, missing verdict artifact, a foreign PR, a symlinked or escaped artifact, malformed closed-shape counts, a blank accepted reason, or a non-completed verdict is a usage error (exit 2) before any template is written. Artifact reads pin the validated run directory descriptor, atomically reject a symlink final component through directory-relative no-follow opens, verify the opened artifact descriptor is regular, and parse from that descriptor. Run identifiers use a conservative ASCII allowlist so they are safe direct-child names and safe to render. Resume rejects self-inheritance before loading because rewriting the source would create circular provenance and destroy the only completed evidence. A malformed resume verdict is reported as `verdict_artifact_corrupt` and preserved. Completed verdicts reject inheritance or disposition regeneration, while an explicit publication-only `--execute` resume branches before verdict-writing validation and republishes the existing evidence without rewriting it. A completed verdict with verdict=BLOCK is an acceptable source — its `accepted` records were validated; only its `must_fix` records are ignored. A genuinely clean completed verdict with no actionable counts remains a valid empty source.
...
The verdict `kind` field is optional on input for migration: legacy artifacts infer pause from the single shared historical pause marker, completed from nonempty findings, and failure otherwise. New artifacts persist the inferred or explicit kind. Legacy verdicts remain readable but cannot auto-carry without a completed inferred kind plus pair and severity equality, current hunk, and hash-of-hashes body-set digests. Verdicts containing an older first-body or delimiter-joined body digest demote through `diagnosis_changed`; absent required source digests demote through `source_digest_missing`. Completed evidence remains immutable during disposition processing and publication-only resume. Publication status migrates a legacy single object into the first element of an append-only array, written through the pinned run descriptor; successful records include inline and body-fallback counts.
