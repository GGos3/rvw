...
`--inherit <run-id>` loads the prior run's `gate-verdict.json` (not its raw `gate-dispositions.yaml`): the verdict is the only artifact whose dispositions passed exact-ID, duplicate, unknown, and owner-authorization validation. The source run's `target.json` must match the current repository and PR number; head/base anchors MAY differ (that is the point). Missing run, missing verdict artifact, a foreign PR, a symlinked or escaped artifact, malformed closed-shape counts, a blank accepted reason, or a pause-stub verdict whose counts still report actionable findings is a usage error (exit 2) before any template is written. Artifact reads validate parent containment, atomically reject a symlink final component, verify the opened descriptor is regular, and parse from that descriptor. Run identifiers use a conservative ASCII allowlist so they are safe direct-child names and safe to render. Resume rejects self-inheritance before loading because rewriting the source would create circular provenance and destroy the only completed evidence. Every resume also rejects an existing completed verdict before inheritance or disposition work; the operator must create a new target run and use inheritance. A completed verdict with verdict=BLOCK is an acceptable source — its `accepted` records were validated; only its `must_fix` records are ignored. A genuinely clean verdict with no actionable counts remains a valid empty source.
...
- **Tier 1 — exact ID plus code-and-diagnosis match**: after confirming the `(file, rule_id)` pair is unique on both sides, the public ID, the persisted SHA-256 digest of the run's canonical unified-diff hunk text, and the persisted SHA-256 digest of the complete order-insensitive collapsed body set must all match. The body digest is an outer SHA-256 over sorted fixed-width inner SHA-256 digest blocks, so body boundaries are unambiguous. The accepted decision and reason then auto-carry, stamped `inherited_from: <run-id>`. A missing source digest, missing current digest, changed hunk, or changed diagnosis demotes with `source_digest_missing`, `current_digest_missing`, `content_changed`, or `diagnosis_changed`, respectively. An empty body set is corrupt merge state and persists an invariant BLOCK outcome.
...
None. New digest and diagnostic fields are optional with defaults; legacy verdicts remain readable but cannot auto-carry without equal current hunk and hash-of-hashes body-set digests. Verdicts containing an older first-body or delimiter-joined body digest demote through `diagnosis_changed`; absent required source digests demote through `source_digest_missing`. Absent `--inherit` reproduces current behavior exactly except that every resume refuses to operate on an already completed verdict.
