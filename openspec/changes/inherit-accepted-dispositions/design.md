...
The verdict `kind` field is optional on input for migration: legacy artifacts infer pause from the historical pause marker, completed from nonempty findings, and failure otherwise. New artifacts persist the inferred or explicit kind. Legacy verdicts remain readable but cannot auto-carry without equal severity, current hunk, and hash-of-hashes body-set digests. Verdicts containing an older first-body or delimiter-joined body digest demote through `diagnosis_changed`; absent required source digests demote through `source_digest_missing`. Completed evidence remains immutable during disposition processing, while an explicit publication-only `--execute` resume can republish it.
